//(function ( $ ) {
var contentValue = "";
function configRichTextBoxForWASM($) {
    ...
    $.fn.richText = function (options) {
            ...
            if ((e.keyCode === 9 || e.keyCode === "9") && e.type === "keydown") {
                ...
                document.addEventListener('keydown', (e) => {
                    ...
                    if (keyPressed['Control']) {
                        if ((e.keyCode === 69 || e.keyCode === 74 || e.keyCode === 76 || e.keyCode === 82) && e.type === "keydown") {
                            alignText(e.keyCode);
                        }
                    }
                });
        ...
        /**
        * Setup Rich text mode
        * @@private
        */
        function ColorTheme() {
            if (eval(IsDarkMode) === true) {
                enableDarkMode();
            }
            else {
                disableDarkMode();
        ...
        /**
         * 
         * Align Text Area elements
         */
        function alignText(selectedKey) {
            var editNode = $('.richText-editor');
            if (selectedKey === 69) {
                align = 'center';
                $btnCenterAlign.css('background-color', 'lightgray');
                $btnJustify.removeAttr('style');
                $btnLeftAlign.removeAttr('style');
                $btnRightAlign.removeAttr('style');
            }
            else if (selectedKey === 74) {
                align = 'justify';
                $btnJustify.css('background-color', 'lightgray');
                $btnLeftAlign.removeAttr('style');
                $btnRightAlign.removeAttr('style');
                $btnCenterAlign.removeAttr('style');
            }
            else if (selectedKey === 76) {
                align = 'left';
                $btnLeftAlign.css('background-color', 'lightgray');
                $btnJustify.removeAttr('style');
                $btnRightAlign.removeAttr('style');
                $btnCenterAlign.removeAttr('style');
            }

            else if (selectedKey === 82) {
                align = 'right';
                $btnRightAlign.css('background-color', 'lightgray');
                $btnJustify.removeAttr('style');
                $btnLeftAlign.removeAttr('style');
                $btnCenterAlign.removeAttr('style');
            }
